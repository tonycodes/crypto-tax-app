...
export class CostBasisCalculator {
  ...
  private calculateTokenCostBasis(
    ...
    const realizedGainLoss = costBasisEntries
      .filter(entry => entry.isDisposed && parseFloat(entry.amount) < 0)
      .reduce((sum, entry) => sum + entry.costBasisUSD, 0)
      .toString();
    ...
    for (const disposition of dispositions) {
      ...
      while (remainingToDispose > 0 && acquisitionIndex < entries.length) {
        ...
        if (availableAmount <= remainingToDispose) {
          ...
          const gainLoss =
            (dispositionPrice - entry.costBasisUSD / availableAmount) * availableAmount;
          ...
          const gainLoss = (dispositionPrice - entry.costBasisUSD / availableAmount) * usedAmount;
          ...
          entry.costBasisUSD =
            (entry.costBasisUSD / availableAmount) * (availableAmount - usedAmount);
