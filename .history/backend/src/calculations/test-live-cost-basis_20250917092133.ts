  ...
  try {
    ...
    for (const method of methods) {
      ...
      for (const result of results) {
        ...
        const originalBuyTxns = transactions.filter(tx => tx.type === 'buy');
        const originalSellTxns = transactions.filter(tx => tx.type === 'sell');

        console.log(`   Original Buy Transactions: ${originalBuyTxns.length}`);
        console.log(`   Original Sell Transactions: ${originalSellTxns.length}`);
        console.log(`   Cost Basis Entries: ${result.entries.length}`);

        if (originalSellTxns.length > 0) {
          console.log(
            `   Sample Sell Transaction: ${originalSellTxns[0]!.amount} ${result.tokenSymbol}`
          );
        }
      }
    }
