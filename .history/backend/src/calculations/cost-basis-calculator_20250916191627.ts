import { Transaction, CostBasisEntry } from '@crypto-tax-app/shared';
...
export class CostBasisCalculator {
  ...
  calculateCostBasis(transactions: Transaction[], options: CalculationOptions): CostBasisResult[] {
    ...
    for (const [tokenSymbol, tokenTransactions] of transactionsByToken) {
   ...
   */
  private sortAcquisitions(acquisitions: Transaction[], method: CostBasisMethod): Transaction[] {
    const sorted = [...acquisitions].sort((a, b) => {
      ...
      switch (method) {
        ...
        case CostBasisMethod.HIFO:
          // Sort by price descending (highest cost first)
          // This is a simplified version - real HIFO would need price data
  ...
  private applyCostBasisMethod(
    ...
    for (const acquisition of acquisitions) {
      entries.push({
        ...
        costBasisUSD: (parseFloat(acquisition.amount) * (acquisition.priceUSD || 0)).toString(),
      ...
      while (remainingToDispose > 0 && acquisitionIndex < entries.length) {
        ...
        if (availableAmount <= remainingToDispose) {
          ...
          entries.push({
            ...
            costBasisUSD: gainLoss.toString(),
