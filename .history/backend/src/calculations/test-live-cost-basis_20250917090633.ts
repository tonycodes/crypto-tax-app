...
// Map adapter TransactionType to Transaction interface type
function mapTransactionType(adapterType: TransactionType): Transaction['type'] {
  switch (adapterType) {
...
async function fetchSolanaTransactions(walletAddress: string): Promise<Transaction[]> {
  ...
  await adapter.initialize({
    rpcUrl: 'https://mainnet.helius-rpc.com/?api-key=demo', // Replace with actual API key in production
    network: 'mainnet',
    rateLimitMs: 200, // Add rate limiting between requests
  });
