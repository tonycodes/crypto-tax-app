...
describe('CostBasisCalculator', () => {
  ...
  describe('calculateCostBasis', () => {
    it('should calculate cost basis for simple buy/sell transactions', () => {
      ...
      expect(result.totalDisposed).toBe('0.5');
      expect(result.remainingQuantity).toBe('0.5');
      expect(result.realizedGainLoss).toBe('250'); // (2500 - 2000) * 0.5
      expect(result.costBasis).toBe('1000'); // 2000 * 0.5 remaining
    });
    ...
    it('should handle LIFO method correctly', () => {
      ...
      console.log('LIFO Test Debug:', {
        ...
        entries: result.entries.map(e => ({
          ...
          isDisposed: e.isDisposed,
        })),
      });
      // LIFO should sell the most recent purchase first (2000), resulting in loss
      expect(result.realizedGainLoss).toBe('-500'); // (1500 - 2000) * 1
      expect(result.remainingQuantity).toBe('1'); // 1 ETH remaining at 1000 cost basis
